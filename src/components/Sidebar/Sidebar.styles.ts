...
export const sidebarStyles = {
  root: (isMinimized: boolean) =>
    cn(
      'flex h-full flex-col bg-sidebar rounded-r-xl pb-6 gap-6 overflow-hidden transition-all duration-300',
  ...
  minimizeIcon: (isMinimized: boolean) =>
    cn('size-6 shrink-0 transition-transform duration-300', isMinimized && 'rotate-180'),
